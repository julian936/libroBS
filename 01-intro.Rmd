...
```{r estacionalidad, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Patrón de precios mensuales por año (interactivo). Se identifica claramente el impacto COVID en marzo 2020 y la recuperación diferenciada por sector."}
...
if(is_html_output()) {
  # VERSION PLOTLY INTERACTIVA
  
  # Colores por año
  colores_años <- c("2019" = "#3498DB", "2020" = "#E74C3C", "2021" = "#27AE60")
  
  # Crear gráficos por sector
  sectores <- unique(datos_mensuales$Sector)
  
  for(sector in sectores) {
    df_sector <- datos_mensuales %>% filter(Sector == sector)
    
    fig <- plot_ly(height = 450)
    
    # Agregar una traza por cada combinación de ticker y año
    for(año in c(2019, 2020, 2021)) {
      for(ticker in unique(df_sector$Ticker)) {
        df <- df_sector %>% filter(Año == año, Ticker == ticker)
        ...
        fig <- fig %>%
          add_trace(
            ...
            type = "scatter",
            mode = "lines+markers",
            name = paste(ticker, año),
            line = list(color = colores_años[as.character(año)], width = 2),
            marker = list(size = 6),
            legendgroup = as.character(año),
            hovertemplate = paste0(
              "<b>", ticker, " - ", año, "</b><br>",
              "Mes: %{x}<br>",
    ...
    fig <- fig %>%
      layout(
        title = list(
          text = paste("Patrón Estacional -", sector, "(2019-2021)"),
          font = list(size = 16)
        ),
        xaxis = list(
          title = "Mes",
...
- **Marzo 2020:** Caída drástica en todos los activos coincidiendo con el crash COVID-19. En tecnología, los precios cayeron bruscamente pero se recuperaron rápidamente. En farmacéuticas, Moderna y Pfizer mostraron volatilidad inicial antes del pico de desarrollo de vacunas.

- **Abril-Mayo 2020:** Período de recuperación diferenciada. El sector tecnológico (especialmente Microsoft y Apple) mostró recuperación en "V", mientras que las farmacéuticas mantuvieron niveles relativamente estables, anticipando su rol en la pandemia.

- **Diciembre 2020 - Enero 2021:** Pico dramático en Moderna alcanzando ~$400 promedio mensual, coincidiendo con la autorización de emergencia y distribución masiva de vacunas. Tesla también experimentó crecimiento explosivo en este período, alcanzando máximos históricos.

- **Segundo semestre 2021:** Normalización post-vacunación. Moderna inicia corrección significativa desde sus máximos, mientras el sector tecnológico consolida ganancias con Microsoft superando los $300 promedio.

- **Diferencias sectoriales:** 
  - **Tecnología**: Patrón de crecimiento sostenido interrumpido brevemente en marzo 2020. Recuperación rápida y tendencia alcista mantenida durante 2020-2021.
  - **Farmacéutica**: Mayor heterogeneidad. J&J mantiene estabilidad (~$150-170), Pfizer presenta volatilidad moderada, y Moderna exhibe el patrón más extremo con auge y corrección dramáticos ligados directamente a eventos de COVID-19.

- **Observación estacional general:** El típico rally de fin de año (noviembre-diciembre) se mantuvo en 2019 y 2021, pero fue interrumpido en 2020 por la dinámica única del sector farmacéutico durante el pico de vacunación.
