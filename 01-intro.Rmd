...
- **Moderna (MRNA):** La mayor volatilidad del dataset (72.1% anual). Crecimiento explosivo durante 
el desarrollo de vacunas (diciembre 2020), alcanzando $484.47 desde $12.26, resultando en un 
retorno total de +44.3%. Posterior corrección drástica reflejando la normalización post-pandemia.
...
```{r sma-tech, echo=FALSE, warning=FALSE, message=FALSE, fig.height=10, results='asis', fig.cap="Series con promedios móviles del sector tecnológico. Las líneas suavizadas (SMA 7, 30, 90 días) revelan las tendencias subyacentes y confirman el crecimiento sostenido post-COVID."}
# Filtrar datos de tecnología
datos_tech <- datos_completos %>% filter(Sector == "Tecnología")
  ...
  for(ticker in c("AAPL", "MSFT", "TSLA")) {
    ...
    fig <- plot_ly(df, height = 450) %>%
      ...
      add_trace(x = ~Fecha, y = ~SMA_7, type = "scatter", mode = "lines",
                ...
                hovertemplate = "SMA 7: $%{y:,.2f}<extra></extra>") %>%
      ...
      add_trace(x = ~Fecha, y = ~SMA_90, type = "scatter", mode = "lines",
                ...
                hovertemplate = "SMA 90: $%{y:,.2f}<extra></extra>") %>%
      layout(
        title = list(text = paste(ticker, "- Promedios Móviles"), font = list(size = 15)),
...
- **SMA 7 días:** Reacciona rápidamente a cambios, útil para identificar reversiones de corto plazo. Sigue de cerca el precio diario pero elimina el ruido extremo.

- **SMA 30 días:** Revela tendencias de mediano plazo, filtrando ruido diario. Útil para traders que buscan mantener posiciones por semanas.

- **SMA 90 días:** Muestra la tendencia de largo plazo, especialmente útil para identificar cambios de régimen durante COVID-19. Esta línea es la más suave y revela la dirección fundamental del mercado.

### Promedios Móviles - Farmacéuticas

```{r sma-pharma, echo=FALSE, warning=FALSE, message=FALSE, fig.height=10, results='asis', fig.cap="Series con promedios móviles del sector farmacéutico. El caso de Moderna es particularmente interesante, mostrando cómo los promedios capturan el ciclo completo de auge y corrección."}
# Filtrar datos de farmacéuticas
datos_pharma <- datos_completos %>% filter(Sector == "Farmacéutica")
...
- **MRNA (Moderna):** Los tres promedios móviles capturan dramáticamente el ciclo completo: ascenso gradual (2020), pico explosivo (dic 2020 - ene 2021), y corrección prolongada (2021-2025). El SMA 90 muestra la tendencia fundamental claramente.

- **PFE (Pfizer):** Comportamiento más estable. Los SMAs revelan que a pesar de volatilidad diaria, la tendencia de largo plazo es relativamente plana con ligera tendencia bajista desde máximos de 2021.

- **JNJ (Johnson & Johnson):** El caso más estable. Los tres SMAs permanecen muy cercanos entre sí, indicando baja volatilidad y crecimiento gradual consistente.
