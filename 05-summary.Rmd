...
```{r prophet-multiples, warning=FALSE, message=FALSE}
# Símbolos de acciones
tickers <- c("AAPL", "MSFT", "TSLA", "PFE", "MRNA", "JNJ")

# Función para ajustar Prophet a una acción (CORREGIDA)
ajustar_prophet_accion <- function(ticker) {
  tryCatch({
    ...
    # Tomar últimas predicciones
    forecast_test <- tail(forecast, n_test)
    test <- test[order(test$ds), ]
    
    # Ajustar longitud
    n_min <- min(nrow(test), nrow(forecast_test))
    test <- tail(test, n_min)
    forecast_test <- tail(forecast_test, n_min)
    ...
    # Eliminar NAs
    idx_validos <- complete.cases(y_real, y_pred)
    y_real <- y_real[idx_validos]
    y_pred <- y_pred[idx_validos]
    
    # Calcular métricas manualmente
    if(length(y_real) > 0) {
      rmse_val <- sqrt(mean((y_real - y_pred)^2))
      mae_val <- mean(abs(y_real - y_pred))
      mape_val <- mean(abs((y_real - y_pred) / y_real)) * 100
      ...
      return(data.frame(ticker = ticker, rmse = NA, mae = NA, mape = NA))
    }
    
  }, error = function(e) {
    cat("Error en", ticker, ":", e$message, "\n")
    return(data.frame(ticker = ticker, rmse = NA, mae = NA, mape = NA))
  })
}

# Aplicar a todas las acciones (usar lapply en lugar de map_dfr)
cat("Procesando", length(tickers), "acciones. Esto puede tomar varios minutos...\n")
lista_resultados <- lapply(tickers, ajustar_prophet_accion)
...
```{r tabla-resultados}
# Mostrar resultados
if(exists("resultados") && nrow(resultados) > 0) {
  knitr::kable(
    resultados,
    col.names = c("Ticker", "RMSE", "MAE", "MAPE (%)"),
    caption = "Métricas de Pronóstico Prophet por Acción (Escala Log)",
    digits = 4,
    align = c('l', 'r', 'r', 'r')
  )
} else {
  cat("No se pudieron calcular los resultados.\n")
}
```
